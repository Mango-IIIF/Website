...
const cdnCode = `<script src="https://cdn.jsdelivr.net/npm/@mango-iiif/iiif-viewer@latest"><\/script>`
  ...
  <div class="inner-page build-detail" :class="`build-detail--${build.accent}`">
    ...
    <section id="demo" class="demo-section page-width">
      <div v-if="build.mode === 'viewer'" class="manifest-loader">
        <div class="manifest-loader__copy">
          ...
          <p>Paste a public IIIF Presentation 2 or 3 manifest URL to load it in Mango.</p>
        </div>
      ...
      <div v-if="build.mode === 'viewer'" class="manifest-presets">
        <div>
          ...
          <p>Choose another IIIF resource to update the viewer.</p>
        </div>
    ...
    <section class="docs-section page-width">
      ...
      <div class="docs-content">
        ...
        <h3>3. Or load Mango from a CDN</h3>
        <p class="docs-content__intro">For a static HTML page, replace the JavaScript import with this script tag. The custom-element markup above remains the same.</p>
        ...
        <div class="callout"><b>Container size matters</b><p>Give <code>&lt;mango-viewer&gt;</code> an explicit height or place it in a flex/grid area with a resolved height. Focused ESM entry points are also available for applications that manage component CSS in their build pipeline.</p></div>
      </div>
    </section>

    <section id="options" class="options-section page-width">
      <div class="section-heading split-heading"><div><span class="eyebrow">Configuration</span><h2>{{ build.shortTitle }} options.</h2></div><p>Pass these values in the <code>config</code> object, or as JSON in the custom element’s <code>config</code> attribute.</p></div>
    ...
    <section class="build-events page-width">
      ...
      <div><p>Mango forwards its internal event bus as standard DOM custom events. Listen on the element and read the payload from <code>event.detail</code>.</p><RouterLink class="text-link" to="/events">Explore all events <span>→</span></RouterLink></div>
    </section>

    <nav class="build-pagination page-width" aria-label="Other builds">
      <RouterLink :to="`/builds/${previous.slug}`"><span>← Previous</span><b>{{ previous.title }}</b></RouterLink>
      <RouterLink :to="`/builds/${next.slug}`"><span>Next →</span><b>{{ next.title }}</b></RouterLink>
    </nav>
  </div>
</template>
