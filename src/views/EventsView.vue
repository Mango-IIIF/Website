...
<template>
  <div class="inner-page">
    <section class="page-hero page-width events-hero">
      ...
      <p>Keep application UI, analytics, and adjacent tools synchronised with navigation, media, annotations, and viewer state.</p>
    </section>

    <section class="docs-section page-width">
      <div class="docs-sidebar"><span class="eyebrow">Listening</span><h2>Use the API that<br />fits your stack.</h2><p>DOM events are ideal in Vue and other frameworks. The JavaScript class offers the same event names with typed callbacks.</p></div>
      ...
      <div v-for="category in eventCategories" :key="category.name" class="event-category">
        ...
        <div class="event-list">
          <article v-for="event in category.events" :key="event[0]">
            <code>{{ event[0] }}</code><code>{{ event[1] }}</code><p>{{ event[2] }}</p>
          </article>
        </div>
      </div>
    </section>
